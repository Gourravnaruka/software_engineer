/* @date 2023-06-29T13:07:22.910463937Z */
...
fmi2Boolean getFmuBoolean(FMUInstance* fmuInstance, const fmi2ValueReference iValueRef) {
	...
	switch(iValueRef) {
		case VRB_Light_light_On_trigger:
			bValue = pPrivateCtx->Light_light_On_trigger;
			pPrivateCtx->Light_light_On_trigger = 0;
			break;
		case VRB_Light_light_Off_trigger:
			bValue = pPrivateCtx->Light_light_Off_trigger;
			pPrivateCtx->Light_light_Off_trigger = 0;
			break;
		case VRB_Light_light_Tick_trigger:
			bValue = pPrivateCtx->Light_light_Tick_trigger;
			pPrivateCtx->Light_light_Tick_trigger = 0;
			break;
		case VRB_Light_light_snd:
			bValue = pPrivateCtx->Light_light_snd;
			break;
		case VRB_Light_InputSignal_Tick_trigger:
			bValue = pPrivateCtx->Light_InputSignal_Tick_trigger;
			pPrivateCtx->Light_InputSignal_Tick_trigger = 0;
			break;
		case VRB_Light_pout_io:
			bValue = pPrivateCtx->Light_pout_io;
			break;
...
void setFmuBoolean(FMUInstance* fmuInstance, const fmi2ValueReference iValueRef, const fmi2Boolean bValue) {
	...
	switch(iValueRef) {
		case VRB_Light_light_On_trigger:
			if(bValue == 1){
				pPrivateCtx->Light_light_On_trigger_isChanged = 1;
				pPrivateCtx->Light_light_On_trigger = bValue;
			}
			break;
		case VRB_Light_light_Off_trigger:
			if(bValue == 1){
				pPrivateCtx->Light_light_Off_trigger_isChanged = 1;
				pPrivateCtx->Light_light_Off_trigger = bValue;
			}
			break;
		case VRB_Light_light_Tick_trigger:
			if(bValue == 1){
				pPrivateCtx->Light_light_Tick_trigger_isChanged = 1;
				pPrivateCtx->Light_light_Tick_trigger = bValue;
			}
			break;
		case VRB_Light_light_snd:
			pPrivateCtx->Light_light_snd_isChanged = 1;
			pPrivateCtx->Light_light_snd = bValue;
			break;
		case VRB_Light_InputSignal_Tick_trigger:
			if(bValue == 1){
				pPrivateCtx->Light_InputSignal_Tick_trigger_isChanged = 1;
				pPrivateCtx->Light_InputSignal_Tick_trigger = bValue;
			}
			break;
		case VRB_Light_pout_io:
			pPrivateCtx->Light_pout_io_isChanged = 1;
			pPrivateCtx->Light_pout_io = bValue;
			break;
...
void fmuInstanciate(FMUInstance* fmuInstance) {
	...
	sPrivateCtx* pPrivateCtx = (sPrivateCtx*)fmuInstance->privateCtx;
	pPrivateCtx->Light_light_On_trigger = 0;
	pPrivateCtx->Light_light_On_trigger_isChanged = 0;
	pPrivateCtx->Light_light_Off_trigger = 0;
	pPrivateCtx->Light_light_Off_trigger_isChanged = 0;
	pPrivateCtx->Light_light_Tick_trigger = 0;
	pPrivateCtx->Light_light_Tick_trigger_isChanged = 0;
	pPrivateCtx->Light_light_snd = 0;
	pPrivateCtx->Light_light_snd_isChanged = 0;
	pPrivateCtx->Light_InputSignal_Tick_trigger = 0;
	pPrivateCtx->Light_InputSignal_Tick_trigger_isChanged = 0;
	pPrivateCtx->Light_pout_io = 0;
	pPrivateCtx->Light_pout_io_isChanged = 0;
...
void RTE_ReadHook_Light_light_On_Start(SignalEvent_On_type* iValue) {
}

void RTE_ReadHook_Light_light_On_Return(SignalEvent_On_type* iValue) {
	sPrivateCtx* pPrivateCtx = (sPrivateCtx*)fmuInst->privateCtx;
	if(pPrivateCtx->Light_light_On_trigger_isChanged == 1){
		(*iValue).trigger = pPrivateCtx->Light_light_On_trigger;
		pPrivateCtx->Light_light_On_trigger_isChanged = 0;
	}
}

void RTE_ReadHook_Light_light_Off_Start(SignalEvent_Off_type* iValue) {
}

void RTE_ReadHook_Light_light_Off_Return(SignalEvent_Off_type* iValue) {
	sPrivateCtx* pPrivateCtx = (sPrivateCtx*)fmuInst->privateCtx;
	if(pPrivateCtx->Light_light_Off_trigger_isChanged == 1){
		(*iValue).trigger = pPrivateCtx->Light_light_Off_trigger;
		pPrivateCtx->Light_light_Off_trigger_isChanged = 0;
	}
}

void RTE_ReadHook_Light_light_Tick_Start(SignalEvent_Tick_type* iValue) {
}

void RTE_ReadHook_Light_light_Tick_Return(SignalEvent_Tick_type* iValue) {
	sPrivateCtx* pPrivateCtx = (sPrivateCtx*)fmuInst->privateCtx;
	if(pPrivateCtx->Light_light_Tick_trigger_isChanged == 1){
		(*iValue).trigger = pPrivateCtx->Light_light_Tick_trigger;
		pPrivateCtx->Light_light_Tick_trigger_isChanged = 0;
	}
}

void RTE_ReadHook_Light_light_snd_Start(Boolean* iValue) {
}

void RTE_ReadHook_Light_light_snd_Return(Boolean* iValue) {
	sPrivateCtx* pPrivateCtx = (sPrivateCtx*)fmuInst->privateCtx;
	if(pPrivateCtx->Light_light_snd_isChanged == 1){
		(*iValue) = pPrivateCtx->Light_light_snd;
		pPrivateCtx->Light_light_snd_isChanged = 0;
	}
}

void RTE_ReadHook_Light_InputSignal_Tick_Start(SignalEvent_Tick_type* iValue) {
}

void RTE_ReadHook_Light_InputSignal_Tick_Return(SignalEvent_Tick_type* iValue) {
	sPrivateCtx* pPrivateCtx = (sPrivateCtx*)fmuInst->privateCtx;
	if(pPrivateCtx->Light_InputSignal_Tick_trigger_isChanged == 1){
		(*iValue).trigger = pPrivateCtx->Light_InputSignal_Tick_trigger;
		pPrivateCtx->Light_InputSignal_Tick_trigger_isChanged = 0;
	}
}

void RTE_WriteHook_Light_pout_io_Start(Boolean* iValue) {
}

void RTE_WriteHook_Light_pout_io_Return(Boolean* iValue) {
	sPrivateCtx* pPrivateCtx = (sPrivateCtx*)fmuInst->privateCtx;
	pPrivateCtx->Light_pout_io = (*iValue);
}
