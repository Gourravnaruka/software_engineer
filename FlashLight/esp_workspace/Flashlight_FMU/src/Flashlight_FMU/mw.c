/* @date 2023-06-29T12:59:08.567804526Z */
...
Rte_Counter_Struct Flashlight_onCount_inst;
...
Integer from_Flashlight_onCount_fp_counts_data;
...
void RTE_Write_Flashlight_onCount_fp_counts_data(Integer* data){
	// Copy byte by byte
	unsigned int byteIdx;

	RTE_WriteHook_Flashlight_onCount_fp_counts_data_Start(&from_Flashlight_onCount_fp_counts_data);
	debug("DEBUG: RTE_Write_Flashlight_onCount_fp_counts_data: Enter\\\n");

	for(byteIdx=0; byteIdx < sizeof(Integer); byteIdx++){
		((char*) &from_Flashlight_onCount_fp_counts_data)[byteIdx] = ((char*) data)[byteIdx];
	}

	debug("DEBUG: RTE_Write_Flashlight_onCount_fp_counts_data: Exit\\\n");
	RTE_WriteHook_Flashlight_onCount_fp_counts_data_Return(&from_Flashlight_onCount_fp_counts_data);
}
...
void RTE_Read_Flashlight_onCount_countin_io(Boolean* data){
	unsigned int byteIdx;

	RTE_ReadHook_Flashlight_onCount_countin_io_Start(&from_Flashlight_light_pout_io);
	RTE_ReadHook_Flashlight_onCount_countin_io_Return(&from_Flashlight_light_pout_io);
	for(byteIdx=0; byteIdx < sizeof(Boolean); byteIdx++){
...
void component_instances_init(void){
  ...
  /* Server call functions */
  Flashlight_timer_inst.GetTime = &RTE_GetTime;
  ...
  /* Server call functions */
  Flashlight_light_inst.GetTime = &RTE_GetTime;
  ...
  /* Server call functions */
  Flashlight_button_inst.GetTime = &RTE_GetTime;
  /* END: Button Rte Instance configuration */

  /* BEG: Counter Rte Instance configuration */
  /* Write functions */
  Flashlight_onCount_inst.Write_fp_counts_data = &RTE_Write_Flashlight_onCount_fp_counts_data;
  /* Trigger functions */
  /* Read functions */
  Flashlight_onCount_inst.Read_countin_io = &RTE_Read_Flashlight_onCount_countin_io;
  /* Server call functions */
  Flashlight_onCount_inst.GetTime = &RTE_GetTime;
  /* END: Counter Rte Instance configuration */
...
void task_descriptors_init(void){
  ...
  task0_rteInstances[3] = &Flashlight_onCount_inst;
...
void components_init(void) {
	...
	Counter_OB_cnt_init(&Flashlight_onCount_inst);
}
